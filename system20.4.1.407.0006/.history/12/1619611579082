...
(select sum(maas) from personel where sirket=sirket_adi) as Maas  
from sirketler ; 
...
SELECT sirket_adi, personel_sayisi, (SELECT ROUND(AVG(maas),2) FROM personel
                                    WHERE sirket_adi=sirket) orta_maas
FROM sirketler;
...
/* -----------------------------------------------------------------------------
  ORNEK8: Her sirketin id’sini, ismini ve toplam kaç şehirde bulunduğunu 
  listeleyen bir SORGU yazınız.
 -------------------------------------------------------------------------*/
 
 SELECT sirket_id,sirket_adi,(SELECT COUNT(sehir) FROM personel
 WHERE sirket_adi=sirket) sehir_sayisi
 FROM sirketler;
